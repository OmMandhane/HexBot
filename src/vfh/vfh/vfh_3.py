...
class VFH(Node):
    def __init__(self, vfh_config):
        ...
        self.safety_distance = vfh_config.get('safety_distance', 0.07)  # Added safety distance
        self.robot_radius = vfh_config.get('robot_radius', 0.035)  # Robot radius
    ...
    def calculate_histogram_field_vector(self, ranges, angles):
        ...
        for i, distance in enumerate(ranges):
            ...
            # Adjusting the histogram based on robot radius and safety distance
            adjusted_distance = max(distance - self.robot_radius - self.safety_distance, 0.1)
            certainty = 100 / (100 + np.exp(-(self.a - self.b * adjusted_distance)))
            obstacle_presence = (certainty ** 2) / adjusted_distance
    ...
    def smooth_histogram(self, histogram):
        ...
        weights = np.concatenate((np.arange(1, self.smoothing_factor + 1), np.arange(self.smoothing_factor + 1, 0, -1))) / float(smoothing_window)
        n = len(histogram)
    ...
    #     v = self.vmax *0.9* (1.0 - (h_current/h_max))
    #     self.threshold = 2.5  #4 working fine
    #     self.get_logger().info(f"VELOCITY: {v} /n THRESH: {self.threshold}")
    #     return v
    def find_velocity(self, teta):
        z = abs(self.calculate_z(math.radians(teta)))
        v = self.vmax * (1.0 - min(z / 0.8, 0.98)) 
        self.get_logger().info(f"VELOCITY: {v} /n z: {z/0.5}")
        # self.threshold = 6
        return v


    def calculate_z(self, steering_direction):
        ...
        return (error * 0.35)	#0.35 is Kp angular velocity coefficient
...
def main(args=None):
    ...
    config = {
        ...
        'sector_size': 5,
        'threshold': 6,
        'safety_distance': 0.07,  # Added
        'robot_radius': 0.035  # Added
    }
